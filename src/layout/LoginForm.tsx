...
import { saveJWT } from '../utils/handleJWT';
...
function LoginForm() {
  ...
  const handleLogin = async () => {
    ...
    */
    try {
      const login = await apiClient.post('/login', data);
      const loginData = { userId: login.data.userId, authToken: login.data.token };
      dispatch(setLoginData(loginData));
      saveJWT(loginData);
      router.push('/chat');
    } catch (error) {
      NotificationFailure('Hubo un error al inciar sesión');
    }
  };
